...
function sanitizeImage(imageUrl) {
    logger.info("ReviewsHelper.sanitizeImage() called")
...
function sanitizeRating(rating) {
    logger.info("ReviewsHelper.sanitizeRating() called")
...
function isString(str) {
    logger.info(`ReviewsHelper.isString called on ${str}`);
    if(typeof str === 'string') {
        logger.info(`${str} is a String`)
        return str;
    } else {
...
function validationMiddleware(req, res, next) {
    logger.info("Validating POST Request Input")
    try {
        if(req.path === '/') {
            if(req.method === 'POST') {
